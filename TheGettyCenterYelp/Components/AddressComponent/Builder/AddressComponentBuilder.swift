...
class AddressComponentBuilder {
    static func buildAddressComponent(withLocation location: BusinessLocation) -> AddressComponentView? {
        guard let view = Bundle.main.loadNibNamed("AddressComponentView", owner: nil, options: nil)?.first as? AddressComponentView else {
            return nil
        }
        let presenter = AddressComponentPresenter(view: view, location: location)
